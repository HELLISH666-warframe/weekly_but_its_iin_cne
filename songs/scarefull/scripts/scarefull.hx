...
introLength = 0;
function postCreate() {
    insert(0,bars = new FlxSprite().loadGraphic(Paths.image("stages/tweak10/scarydeath/bars"))).camera = camHUD;

    dad.visible = false;

    camGame.visible = false;
    //strumLines.members[0].characters[0].alpha=0;
    strumLines.members[2].characters[0].setPosition(450,90);
    strumLines.members[2].characters[0].visible=false;

    strumLines.members[3].characters[0].setPosition(1050,275);
    strumLines.members[3].characters[0].visible=false;

    strumLines.members[4].characters[0].setPosition(612.5,275);
    insert(8,strumLines.members[4].characters[0]);
}
