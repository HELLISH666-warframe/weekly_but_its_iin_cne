...
function onLoad()
{
    ...
    ExUtils.addShader(rainShader, camGame);
...
function onUpdate(elapsed:Float)
{
    ...
    rainShader.setFloat('uIntensity', rainIntensity);
    dadGroup.y = FlxMath.lerp(dadGroup.y, shaggyY + (Math.cos(s) * 65), FlxMath.bound(1, 0, elapsed * 4));
    boyfriendGroup.y = FlxMath.lerp(boyfriendGroup.y, gokuY + (Math.cos(s) * 65), FlxMath.bound(1, 0, elapsed * 4));
    zero.y = FlxMath.lerp(zero.y, zeroY + (Math.cos(s - 5) * 45), FlxMath.bound(1, 0, elapsed * 4));
    ...
    for(anim in extraChartAnims){
        if(anim.time <= Conductor.songPosition){
            var animToPlay:String = singAnimations[anim.data % 4];
            var char:Character;
            if (anim.data > 3)
            {
                if(forceSection)
    ...
    if (name == 'Tenkaichi Events')
    {
        switch (v1) 
        {
            ...
            case 'cameo':  
                ...
                trace(cameoNum);
            case 'emotional':
                ExUtils.removeShader(rainShader, camGame);
                ...
                dad.shader = shaggysil;
                gf.shader = guestsil;
                boyfriend.shader = gokusil;
...
function goodNoteHit(note:Note)
{
    ...
    {
        spiderman.playAnim(singAnimations[note.noteData % 4], true);
