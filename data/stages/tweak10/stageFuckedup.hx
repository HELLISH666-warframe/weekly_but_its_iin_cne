...
function postCreate(){
    ...
    logo = new FlxSprite().loadGraphic(Paths.image("stages/tweak10/genrenull/logo"));
    logo.camera = camHUD;
    ...
    shit = [scoreTxt,missesTxt,accuracyTxt, timeBar, timeBarBG, timeTxt, healthBarBG, healthBar, iconP1, iconP2];
    if(!skipIntro){
        skipCountdown = true;
...
function onEventPush(event){
    if(event.event == 'Lyric Special'){
        ...
        text.setFormat(Paths.font("vcr.ttf"), 64, 0xFF0eeadf, 'center', FlxTextBorderStyle.OUTLINE, FlxColor.WHITE);
        text.updateHitbox();
