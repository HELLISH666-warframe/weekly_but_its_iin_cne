...
function create() {
    ...
    space.loadGraphic(Paths.image("stages/genrenull/space"));
    space.antialiasing = true;
    space.scrollFactor.set(0.5, 0.5);
    insert(1, space);
	add(space);

    miku = new FlxAnimate(3350, 300, 'images/stages/genrenull/miku'); //Gulp
    ...
    backdebris.loadGraphic(Paths.image("stages/genrenull/backdebris"));
...
*/
    ...
    fullstage.loadGraphic(Paths.image("stages/genrenull/fullstage"));
    fullstage.antialiasing = true;
    insert(4, fullstage);
	add(fullstage);

    camandmic = new FlxSprite(2000, 3250);
    camandmic.loadGraphic(Paths.image("stages/genrenull/camandmic"));
...
function postCreate(){
    ...
    logo = new FlxSprite().loadGraphic(Paths.image("stages/genrenull/logo"));
    logo.camera = camHUD;
...
function onSongStart(){
    if(!skipIntro){
        ...
        camFollow.getPosition(5250, 1800);
    
    
        camZooming = false;
        camGame.zoom = 1.5;
        FlxTween.num(camGame.zoom, 0.5, time, {ease: FlxEase.quadInOut, onUpdate: (s)->{
...
override function update(elapsed:Float){
    ...
    // grad.update(elapsed);
    s += elapsed;
        backdebris.y = FlxMath.lerp(backdebris.y, skY + (Math.cos(s - 5) * 65), FlxMath.bound(1, 0, elapsed * 4));
        camandmic.y = FlxMath.lerp(camandmic.y, camandmicY + (Math.cos(s) * 65), FlxMath.bound(1, 0, elapsed * 4));  
}
